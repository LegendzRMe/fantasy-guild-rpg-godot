...
static func run()->Array:
	...
	var materials:=empty_state(2);var material_result:=InventorySystem.add_material(materials,"ore",98);InventorySystem.add_material(materials,"herbs",100);var partial:=InventorySystem.add_material(materials,"ore",5)
	TestSupport.check(errors,material_result.success and partial.collected==2 and partial.rejected==3 and InventorySystem.occupied_count(materials)==2,"Materials should fill an existing stack and report a safe partial collection when the Vault is full.")
	TestSupport.check(errors,InventorySystem.empty_slots(materials)==0 and InventorySystem.warning_state(materials)=="critical","A full Vault should expose the critical capacity state.")
	...
	TestSupport.check(errors,not InventorySystem.add_equipment(materials,item("pinewatch_bow","blocked_bow")).success,"New equipment should require an empty slot.")
	...
	var not_freed:=empty_state(1);InventorySystem.add_material(not_freed,"ore",100);var blocked:=InventorySystem.simulate_inventory_transaction(not_freed,[{"kind":"material","material_id":"ore","quantity":8}],[{"kind":"equipment","item":item("pinewatch_bow","blocked_craft")}])
	...
	TestSupport.check(errors,multi_result.success and InventorySystem.occupied_count(multi)==2,"Multi-input, multi-output inventory transactions should commit atomically when all outputs fit.")
