...
static func run() -> Array:
	...
	TestSupport.check(errors,opening_roles.count("Swift")==2 and float(opening.enemy_health_overrides.Swift)==GameData.CLASSES.Guardian.damage*2.0,"Encounter 1 should contain two Swifts that fall to two founding-Guardian hits.")
	TestSupport.check(errors,float(opening.enemy_health_overrides.Raider)==GameData.CLASSES.Guardian.damage*5.0 and float(opening.enemy_health_overrides.Archer)==GameData.CLASSES.Guardian.damage*5.0,"Regular Encounter 1 enemies should take about five founding-Guardian hits.")
	TestSupport.check(errors,opening_roles.count("Stalker")==1 and float(opening.enemy_health_overrides.Stalker)<=GameData.CLASSES.Guardian.damage,"Encounter 1 should introduce exactly one very weak Tank-ignoring Stalker.")
	...
	TestSupport.check(errors,bool(GameData.ENEMIES.Stalker.prefers_backline) and bool(GameData.ENEMIES.Stalker.ignores_tank_aggro) and float(GameData.ENEMIES.Stalker.base_damage)<16.0,"Stalkers should use the distinct backline Fixate behavior that ignores Tank aggro while remaining a weak attacker.")
