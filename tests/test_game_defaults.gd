...
static func run() -> Array:
	var errors:=[]
	TestSupport.check(errors,GameData.CLASSES.keys()==["Guardian","Cleric","Rogue","Ranger","Mage","Warlock","Slayer","Priest","Shaman"],"Established classes should retain their order and append Shaman.")
	TestSupport.check(errors,GameData.ABILITIES.size()==9 and GameData.ABILITY_TARGETING.size()==9,"Every current class should retain ability and targeting definitions.")
