...
static func run() -> Array:
	var errors:=[]
	TestSupport.check(errors,GameData.CLASSES.keys()==["Guardian","Cleric","Rogue","Ranger","Mage","Warlock","Slayer","Priest","Shaman","Templar","Protector"],"Established classes should retain their order and append Protector.")
	TestSupport.check(errors,GameData.ABILITIES.size()==11 and GameData.ABILITY_TARGETING.size()==11,"Every current class should retain ability and targeting definitions.")
