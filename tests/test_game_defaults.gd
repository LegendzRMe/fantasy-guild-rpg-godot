...
static func run() -> Array:
	var errors:=[]
	TestSupport.check(errors,GameData.CLASSES.keys()==["Guardian","Cleric","Rogue","Ranger","Mage","Warlock","Slayer","Priest","Shaman","Templar","Protector","Sentinel","Huntsman","Druid","Warrior"],"Established classes should retain their order and append Warrior after Druid.")
	TestSupport.check(errors,GameData.ABILITIES.size()==15 and GameData.ABILITY_TARGETING.size()==15,"Every current class should retain ability and targeting definitions.")
