...
static func run() -> Array:
	var errors:=[]
	TestSupport.check(errors,GameData.CLASSES.keys()==["Guardian","Cleric","Rogue","Ranger","Mage","Warlock","Slayer","Priest","Shaman","Templar","Protector","Sentinel","Huntsman","Druid","Warrior","Death Knight"],"Established classes should retain their order and append Death Knight after Warrior.")
	TestSupport.check(errors,GameData.ABILITIES.size()==16 and GameData.ABILITY_TARGETING.size()==16,"Every current class should retain ability and targeting definitions.")
