...
static func run() -> Array:
	var errors:=[]
	TestSupport.check(errors,GameData.CLASSES.keys()==["Guardian","Cleric","Rogue","Ranger","Mage","Warlock","Slayer","Priest","Shaman","Templar","Protector","Sentinel"],"Established classes should retain their order and append Sentinel.")
	TestSupport.check(errors,GameData.ABILITIES.size()==12 and GameData.ABILITY_TARGETING.size()==12,"Every current class should retain ability and targeting definitions.")
