...
static func run() -> Array:
	var errors:=[]
	TestSupport.check(errors,GameData.CLASSES.keys()==["Guardian","Cleric","Rogue","Ranger","Mage","Warlock","Slayer","Priest"],"Established classes should retain their order and append Priest.")
	TestSupport.check(errors,GameData.ABILITIES.size()==8 and GameData.ABILITY_TARGETING.size()==8,"Every current class should retain ability and targeting definitions.")
