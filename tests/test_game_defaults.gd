...
static func run() -> Array:
	var errors:=[]
	TestSupport.check(errors,GameData.CLASSES.keys()==["Guardian","Cleric","Rogue","Ranger","Mage","Warlock","Slayer"],"Established classes should retain their order and append the testing-only Slayer.")
	TestSupport.check(errors,GameData.ABILITIES.size()==7 and GameData.ABILITY_TARGETING.size()==7,"Every current class should retain ability and targeting definitions.")
