...
static func run() -> Array:
	var errors:=[]
	TestSupport.check(errors,GameData.CLASSES.keys()==["Guardian","Cleric","Rogue","Ranger","Mage","Warlock","Slayer","Priest","Shaman","Templar"],"Established classes should retain their order and append Templar.")
	TestSupport.check(errors,GameData.ABILITIES.size()==10 and GameData.ABILITY_TARGETING.size()==10,"Every current class should retain ability and targeting definitions.")
