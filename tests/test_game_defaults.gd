...
static func run() -> Array:
	var errors:=[]
	TestSupport.check(errors,GameData.CLASSES.keys()==["Guardian","Cleric","Rogue","Ranger","Mage","Warlock","Slayer","Priest","Shaman","Templar","Protector","Sentinel","Huntsman","Druid","Warrior","Death Knight","Beastmaster","Monk","Paladin","Crusader","Vanguard","Vitalist","Spirit Weaver"],"Established classes should retain their order and append Spirit Weaver after Vitalist.")
	TestSupport.check(errors,GameData.ABILITIES.size()==23 and GameData.ABILITY_TARGETING.size()==23,"Every current class should retain ability and targeting definitions.")
