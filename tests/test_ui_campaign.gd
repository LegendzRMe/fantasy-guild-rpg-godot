...
static func run(main:Node)->Array:
	...
	var class_range_names:Array[String]=["TestingWarlockRangeStart","TestingRogueRangeStart","TestingSlayerRangeStart","TestingPriestRangeStart","TestingShamanRangeStart","TestingTemplarRangeStart","TestingProtectorRangeStart","TestingSentinelRangeStart"]
	var class_range_buttons:Array=class_range_names.map(func(control_name:String):return main.ui.find_child(control_name,true,false))
	TestSupport.check(errors,main.screen=="combat_hall" and main.ui.find_child("CombatHall4v4Start",true,false)!=null and main.ui.find_child("CombatHall8v8Start",true,false)!=null and main.ui.find_child("TestingDummyRangeStart",true,false)!=null and class_range_buttons.all(func(control):return control!=null) and main.ui.find_child("TestingEndlessStart",true,false)!=null,"The Combat Hall should keep every class range, raid, scrimmage, dummy-range, and Endless Arena launch action available.")
	TestSupport.check(errors,class_range_buttons.all(func(control):return control.get_parent() is HFlowContainer),"Class-range launch actions should use a wrapping flow layout instead of overflowing the Combat Hall horizontally.")
