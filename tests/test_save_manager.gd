...
static func run() -> Array:
	...
	TestSupport.check(errors,testing.heroes.size()==10,"The testing slot should include all current normal recruits, Slayer, and Special Hero candidates.")
	...
	TestSupport.check(errors,testing.heroes.slice(0,7).all(func(hero):return hero.prestige_rank==0) and testing.heroes.slice(7).all(func(hero):return hero.prestige_rank==1),"Normal heroes should begin at Prestige 0 while current Special Heroes begin at Prestige 1.")
	TestSupport.check(errors,testing.heroes.slice(7).all(func(hero):return hero.identity_type=="special" and not hero.can_edit_name and not hero.can_edit_appearance),"Special Heroes should preserve authored, non-editable identities.")
