...
static func run() -> Array:
	...
	TestSupport.check(errors,testing.heroes.size()==11,"The testing slot should include all current normal recruits, Slayer, Priest, and Special Hero candidates.")
	...
	TestSupport.check(errors,testing.heroes.slice(0,8).all(func(hero):return hero.prestige_rank==0) and testing.heroes.slice(8).all(func(hero):return hero.prestige_rank==1),"Normal heroes should begin at Prestige 0 while current Special Heroes begin at Prestige 1.")
	TestSupport.check(errors,testing.heroes.slice(8).all(func(hero):return hero.identity_type=="special" and not hero.can_edit_name and not hero.can_edit_appearance),"Special Heroes should preserve authored, non-editable identities.")
	...
	TestSupport.check(errors,legacy_testing.heroes.size()==11 and legacy_testing.heroes.any(func(hero):return str(hero.get("class",""))=="Slayer") and legacy_testing.heroes.any(func(hero):return str(hero.get("class",""))=="Priest") and legacy_testing.heroes.any(func(hero):return str(hero.get("name",""))=="Ilyra Voss"),"Legacy testing saves should automatically receive every current testing hero and class without duplicating existing heroes.")
