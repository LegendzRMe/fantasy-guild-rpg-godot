...
static func run(main:Node) -> Array:
	...
	TestSupport.check(errors,main.state.tutorial_complete==true and main.state.heroes.size()==22,"The testing slot should bypass the tutorial with its complete roster.")
	await main.get_tree().process_frame
