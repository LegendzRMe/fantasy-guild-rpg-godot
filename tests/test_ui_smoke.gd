...
static func run(main:Node) -> Array:
	...
	if mage_battle_index>=0:
		...
		main.cast_mage_q(visual_mage,flame_target.pos);main.update_mage_runtime(.9);TestSupport.check(errors,is_equal_approx(flame_target.hp,flame_health_before),"Flamestrike should preserve its full one-second warning before damage.")
		main.update_mage_runtime(.2);TestSupport.check(errors,flame_target.hp<flame_health_before and main.effects.any(func(effect):return effect.kind=="danger"),"Flamestrike should resolve after its readable warning at the chosen ground point.")
