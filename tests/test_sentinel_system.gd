...
static func run()->Array:
	...
	var innervated:=unit();AbilitySlotSystem.spend(innervated.sentinel_runtime.q_slot);AbilitySlotSystem.spend(innervated.sentinel_runtime.w_slot);var q_before:float=float(innervated.sentinel_runtime.q_slot.timers[0]);var w_before:float=float(innervated.sentinel_runtime.w_slot.timers[0]);SentinelSystem.update(innervated,2.0,1.5,1.5)
	TestSupport.check(errors,is_equal_approx(float(innervated.sentinel_runtime.q_slot.timers[0]),q_before-3.0) and is_equal_approx(float(innervated.sentinel_runtime.w_slot.timers[0]),w_before-3.0),"External cooldown recovery should accelerate Sentinel Q and W charge slots.")
