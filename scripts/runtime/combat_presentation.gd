...
func draw_combat_debug_overlay()->void:
	...
	if str(hero.get("class",""))=="Druid" and not hero.get("druid_runtime",{}).is_empty():
		...
		lines.append("LEVEL %d  POWER %.1f  HP %.1f%%"%[int(hero.level),float(hero.power),DruidSystem.health_ratio(hero)*100.0]);lines.append("BA %.1f / %.2fs  TARGET %s  DIST %.1f"%[float(hero.damage),float(hero.basic_attack_interval),str(druid.designated_ally_id),ally_distance]);lines.append("MINI HOTS %d  %s"%[druid.mini_hots.size(),str(druid.mini_hots.map(func(hot):return "%.2f"%float(hot.remaining_duration)))]);lines.append("REGROWTH %s"%str(druid.regrowths.map(func(hot):return "%s %.2f"%[str(hot.target_id),float(hot.remaining_duration)])));lines.append("TICK %.1f  Q/W/E %.1f/%.1f/%.1f"%[DruidSystem.regrowth_tick_request(hero),float(hero.ability_cds[0]),float(hero.ability_cds[1]),float(hero.ability_cds[2])]);lines.append("INNERVATE %d/%d  RECHARGE x%.2f  REVIT %.1f"%[int(druid.d_slot.current_charges),int(druid.d_slot.max_charges),DruidSystem.innervate_recharge_rate(hero),float(druid.revitalize_remaining)]);lines.append("ROOTS %d  QUEST %d  TREANTS %d  DMG %.1f"%[druid.roots_areas.size(),int(druid.vengeful_quest_stacks),druid.treants.size(),DruidSystem.treant_damage(hero)]);lines.append("TRANQ %.1f  TWILIGHT %.1f  SHOWER %d"%[float(druid.tranquility_remaining),float(druid.twilight_pending),int(druid.lunar_shower_stacks)]);lines.append("CURE %d  HEAL/OVER %.1f/%.1f  COMM %.1f"%[int(druid.recent_cure_count),float(druid.recent_healing),float(druid.recent_overhealing),float(druid.recent_communion_snapshot)]);lines.append("TELEMETRY  %s"%str(druid.telemetry))
