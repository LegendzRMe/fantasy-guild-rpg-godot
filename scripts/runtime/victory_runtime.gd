...
func finish_battle(win:bool)->void:
	for hero in heroes:
		...
		if str(hero.get("class",""))=="Death Knight" and not hero.get("death_knight_runtime",{}).is_empty() and int(hero.get("hero_index",-1))>=0 and int(hero.hero_index)<state.heroes.size():state.heroes[int(hero.hero_index)]["talent_mastery"]=hero.death_knight_runtime.mastery.duplicate(true)
	battle_over=true
