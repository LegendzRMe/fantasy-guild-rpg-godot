extends "res://scripts/runtime/spiritweaver_runtime.gd"
...
func use_ability(slot:int,cast_position:Vector2=Vector2.INF,item_repeat:bool=false) -> void:
	...
	var spiritweaver_trait_input:bool=str(h.get("class",""))=="Spirit Weaver" and slot==4
	if not protector_trait_input and not sentinel_trait_input and not druid_trait_input and not warrior_trait_input and not death_knight_trait_input and not beastmaster_trait_input and not monk_trait_input and not paladin_trait_input and not crusader_trait_input and not vanguard_trait_input and not vitalist_trait_input and not spiritweaver_trait_input and not TalentSystem.ability_is_unlocked(int(state.heroes[battle_hero_indices[selected]].level),slot):return
	var ability_enemy_target:int=combat_enemy_target()
	if slot>=4 and h["class"] not in ["Protector","Sentinel","Druid","Warrior","Death Knight","Beastmaster","Monk","Paladin","Crusader","Vanguard","Vitalist","Spirit Weaver"]:return
	if h.hp<=0:return
	if not item_repeat:
		...
		elif h["class"]=="Spirit Weaver" and slot==4 and spiritweaver_trait_input:pass
		elif h["class"]=="Spirit Weaver" and slot==2 and SpiritWeaverSystem.has_talent(h,"spiritweaver_l9_2") and not h.spiritweaver_runtime.totem.is_empty() and not bool(h.spiritweaver_runtime.totem.repositioned):pass
	...
	if h["class"]=="Vitalist":
		...
		return
	if h["class"]=="Spirit Weaver":
		cast_spiritweaver_ability(slot,cast_position)
		return
