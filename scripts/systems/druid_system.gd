...
static func initialize_runtime(unit:Dictionary,telemetry_enabled:bool=false,encounter_id:String="")->void:
	unit["druid_runtime"]={
		...
		"twilight_pending":0.0,"twilight_point":Vector2.ZERO,"astral_pending":0.0,"astral_point":Vector2.ZERO,
...
static func apply_regrowth(unit:Dictionary,target:Dictionary,direct:bool=true)->Dictionary:
	...
	var removed:Array=[]
	if direct and has_talent(unit,"druid_l18_3"):
...
static func moonfire_plan(unit:Dictionary,contacts:Array,free_cast:bool=false)->Dictionary:
	var unique:Dictionary={}
	for target in contacts:
		if TargetCategorySystem.qualifies_immediate(target):unique[target_id(target)]=target
	var count:=mini(unique.size(),int(DruidData.VALUES.moonfire_cap));var regrowth_count:=active_regrowths(unit).size()
...
static func roots_radius(unit:Dictionary,elapsed:float,secondary:bool=false)->float:
	...
	var radius:=lerpf(float(DruidData.SPACE.roots_initial_radius),float(DruidData.SPACE.roots_max_radius),progress)
	return radius*(1.0+float(DruidData.VALUES.deep_roots_size) if has_talent(unit,"druid_l9_1") and not secondary else 1.0)
...
static func create_roots_area(unit:Dictionary,point:Vector2,secondary:bool=false)->Dictionary:
	...
	return {"cast_id":next_cast_id(unit,"roots_secondary" if secondary else "roots"),"point":point,"elapsed":0.0,"remaining":float(DruidData.VALUES.roots_growth)+persistence,"contact_ids":[],"secondary":secondary,"verdant_fired":false}

static func note_root_result(unit:Dictionary,target:Dictionary,applied:bool,secondary:bool=false)->Dictionary:
	...
	var emerald:=float(DruidData.VALUES.emerald_cdr) if has_talent(unit,"druid_l9_3") else 0.0
...
static func innervate_recharge_rate(unit:Dictionary)->float:
	if not has_talent(unit,"druid_l12_3"):return 1.0
	return 1.0+active_regrowths(unit).size()*float(DruidData.VALUES.shando_regrowth_rate)
...
static func cast_innervate(unit:Dictionary,target:Dictionary)->Dictionary:
	...
	if has_talent(unit,"druid_l30_3") and regrowth!=null:
		communion=power_scaled(unit,PeriodicStatusSystem.remaining_scheduled_amount(regrowth)*float(DruidData.VALUES.nature_communion_remaining))*hot_multiplier(unit,["healing_over_time","regrowth"])
...
static func advance(unit:Dictionary,delta:float)->Dictionary:
	...
	AbilitySlotSystem.update(runtime.d_slot,delta,innervate_recharge_rate(unit))
