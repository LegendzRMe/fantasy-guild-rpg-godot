...
enum RechargeMode { SEQUENTIAL, INDEPENDENT, FULL_REFILL }
...
static func update(slot:Dictionary,delta:float,rate:float=1.0)->void:
	...
	elif mode==RechargeMode.FULL_REFILL and not slot.timers.is_empty():
		slot.timers[0]=float(slot.timers[0])-step
		if float(slot.timers[0])<=0.0:slot.timers.clear();slot.current_charges=int(slot.max_charges)
